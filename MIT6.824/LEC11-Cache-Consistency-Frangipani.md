...
对于Petal中所保存的数据（元数据、目录、数据块等）来说都有一个版本号，当修改元数据并保存更新后的数据的时候会对版本+1。如果数据块的版本号大于等于log entry中的版本号，ws3就忽略该日志，所以上述场景ws3不会执行ws1删除的日志。
当ws3进行恢复工作的时候，ws2可能持有着该目录所对应的锁，不能对目录