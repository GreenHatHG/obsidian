...
var MyPlugin = /** @class */ (function (_super) {
    ...
    */
    ...
    MyPlugin.prototype.自动设置标题 = function() {        
        ...
        if (!笔记全文) return;
        笔记全文 = 笔记全文.replace(/\r?\n/g,"↫");
        笔记全文 = 笔记全文.replace(/↫\s*↫/g,"↫↫");
        笔记全文 = 笔记全文.replace(/\s*(?=↫)/g,"");
        笔记全文 = 笔记全文.replace(/(?<=^|↫)([^#`\[\]\(\)↫]{3,}[^\.\?\!:,0-9，：。？！）↫])(?=↫|$)/g,"↫### $1↫");
        笔记全文 = 笔记全文.replace(/#+([^#↫]+)↫*$/mg,"$1");    //取消末行标题
        笔记全文 = 笔记全文.replace(/↫{3,}/g,"\r\n\r\n");
        笔记全文 = 笔记全文.replace(/↫/g,"\r\n");
        this.替换笔记全文 (笔记全文);
    };
    ...
    MyPlugin.prototype.获取标注文本 = function() {        
        this.获取编辑器信息 ();
        if (!笔记全文) return;
        var tmp = 笔记全文.replace(/^(?!#+ |#注释|#标注|#批注|#反思|#备注|.*==|.*%%).*$|^[^#\n%=]*(==|%%)|(==|%%)[^\n%=]*$|(==|%%)[^\n%=]*(==|%%)/mg,"\n");
        tmp = tmp.replace(/[\r\n|\n]+/g,"\n")        
    ...
    MyPlugin.prototype.添加中英间隔 = function() {
        this.获取编辑器信息 ();
        if (!笔记全文) return;
        笔记全文 = 笔记全文.replace(/([a-zA-Z]+)([一-鿆]+)/g,"$1 $2")
        笔记全文 = 笔记全文.replace(/([一-鿆]+)([a-zA-Z]+)/g,"$1 $2")
        this.替换笔记全文 (笔记全文);
    };
    ...
    MyPlugin.prototype.修复错误语法 = function() {        
        this.获取编辑器信息 ();
        if (!笔记全文) return;
        笔记全文 = 笔记全文.replace(/[\[【]([^\[\]【】]*)[】\]][（|\(]([^\(\)（）]*)[）|\)]/g,"\[$1\]\($2\)");
        //将 【】（）或【】() 转换为[]()
        笔记全文 = 笔记全文.replace(/\[+([^\[\]]*)\]+\(/g,"\[$1\](");
        //处理 bookXnote 回链语法，将 [[链接]]() 转换为 []()
        笔记全文 = 笔记全文.replace(/(?<=^|\s)    /mg,"\t");
        //把 四个空格转换为 制表符
        笔记全文 = 笔记全文.replace(/\*\s+\>\s+/g,"- ");
        //处理 bookXnote 回链语法中的列表
        笔记全文 = 笔记全文.replace(/(?<=\s)[0-9]+。 /g,"1. ");
        //把 1。 转换为 有序列表样式
        this.替换笔记全文 (笔记全文);
    };
    ...
    MyPlugin.prototype.英转中文标点 = function() {        
        this.获取编辑器信息 ();
        if (!笔记全文) return;
        笔记全文 = 笔记全文.replace(/(?<=[^a-z0-9\>\]]),/g,"，")
        笔记全文 = 笔记全文.replace(/(?<=[^a-z0-9\>\]])\./isg,"。");   //数字后的黑点不转为句号
        笔记全文 = 笔记全文.replace(/(?<=[^a-z0-9\>\]])\?/g,"？")
        笔记全文 = 笔记全文.replace(/(?<=[^a-z0-9\>\]])!(?=[^\[])/g,"！")
        笔记全文 = 笔记全文.replace(/;/g,"；")
        笔记全文 = 笔记全文.replace(/(?<=[^:]):|:(?=[^:])/g,"：")
    ...
    MyPlugin.prototype.转换路径 = function() {        
        ...
        if (link1.test(笔记全文)){
            笔记全文 = 笔记全文.replace(/\s/mg,"%20");
            笔记全文 = 笔记全文.replace(/^(.*)$/m,"\[file\]\(file:///$1\)");
            笔记全文 = 笔记全文.replace(/\\/img,"\/");
            this.替换笔记全文 (笔记全文);
        }else if(link2.test(笔记全文)){
            笔记全文 = 笔记全文.replace(/%20/mg," ");
            笔记全文 = 笔记全文.replace(/^(\[[^\[\]]*\]\()*file:\/\/\/([^\(\)]*)\)*/m,"$2");
            笔记全文 = 笔记全文.replace(/\//mg,"\\");
            this.替换笔记全文 (笔记全文);
        }else if(link3.test(笔记全文)){
            笔记全文 = 笔记全文.replace(/^\[[^\[\]]*\]\(([a-zA-Z]:\\[^\(\)]*)\)*/m,"$1");
            this.替换笔记全文 (笔记全文);
        }else{
    ...
    MyPlugin.prototype.简体转繁 = function(){        
        ...
        for (var i=0;i<_list1.length;i++){ 
            笔记全文 = 笔记全文.replace(eval("/"+_list1[i]+"/g"),_list2[i]);
        }
        this.替换笔记全文 (笔记全文);
    }
    ...
    MyPlugin.prototype.繁体转简 = function(){        
        ...
        var _list2 = this.settings.ftList;        
        for (var i=0;i<_list1.length;i++){ 
            笔记全文 = 笔记全文.replace(eval("/"+_list2[i]+"/g"),_list1[i]);
        }
        this.替换笔记全文 (笔记全文);
    }

    /*
    以下为基本函数，涉及 获取所选文本、获取笔记正文、替换所选文本、替换笔记全文、获取编辑模式
    */
    ...
    MyPlugin.prototype.替换笔记全文 = function(lines) {
        var cmEditor = this.获取编辑模式 ();
        if (cmEditor.getSelection() != "") {
...
var SettingsTab = /** @class */ (function (_super) {
    ...
    SettingsTab.prototype.display = function () {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: '增强编辑 0.4.0' });
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc('可将[( (< ([ "[ \'[等组合转为〖〈〔『「，或将dv、qy、mm、CSS、js、ty等单词转为代码块语法。')
        new obsidian.Setting(containerEl)
            .setName('智能换行「Enter」 编辑区内根据上下文特点进行智能换行')
            .setDesc('开启后，非列表或代码块的文本中按下回车补加一次换行;如想普通换行，可按下 Shift+Enter 。')
        ...
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('转换斜体语法「Alt+X」∶将选文转为或去除 *斜体* 效果');
        donateText.appendChild(document.createElement('br'));
        ...
        donateText.appendChild(document.createElement('br'));
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('删除当前段落「Ctrl+D」∶去除当前段落文本;若在[[]]内会先删除链接内容;当遇有序列表项时会正常调小后面序号。');
        donateText.appendChild(document.createElement('br'));
